...
import com.tdr.familytreasure.amap.CoordinateUtil;
...
public class FootprintActivity extends BackTitleActivity implements AMap.OnMapLoadedListener, AMap
    ...
    private void addPolyLine(List<GetElderMonitorData.ContentBean.MONITORDATALISTBean> footprints) {
        ...
        for (GetElderMonitorData.ContentBean.MONITORDATALISTBean footprint : footprints) {
            latLngs.add(gps2Gaode(footprint.getLAT(), footprint.getLNG()));
        }
        mAmap.addPolyline(new PolylineOptions().setCustomTexture(BitmapDescriptorFactory.fromResource(R.mipmap
                .arrow_footprint)).
...
//            markerOption.position(new LatLng(footprints.get(i).getLAT(), footprints.get(i).getLNG()));
            markerOption.position(gps2Gaode(footprints.get(i).getLAT(), footprints.get(i).getLNG()));
    ...
    private LatLng gps2Gaode(double lat, double lng) {
        Log.e(TAG, "前 lat: "+lat+"lng: "+lng );
        Location location = new Location("");
        location.setLatitude(lat);
        location.setLongitude(lng);
        LatLng latLng = new LatLng(location.getLatitude(), location.getLongitude());
        latLng = CoordinateUtil.transformFromWGSToGCJ(latLng);
        Log.e(TAG, "后 lat: "+latLng.latitude+"lng: "+latLng.longitude );
        return latLng;
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
        ...
        mAmap.moveCamera(CameraUpdateFactory.changeLatLng(new LatLng(footprint.getLAT(), footprint.getLNG())));
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                markers.get(position).setToTop();
